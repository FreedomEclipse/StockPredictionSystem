...
class BreakPage(tk.Frame):
    ...
    def main_frame(self, pNum, questions, *args):
        if pNum == 15:
            main_text = "Thank you for completing this series of questions. Take a minute to relax and" \
                        "\n\n then algebra questions will appear.  " \
                        "Solve each equation mentally.  When done, click to select your answer.\n\n" \
                        "A pause will occur after each question and a spinning colored circle may appear.\n\n" \
                        "Relax until the next question appears."
        elif pNum == 37:
...
class ScalePage(tk.Frame):
    ...
    def main_frame(self, pNum, questions, *args):
        main_text = "Please rate your willingness to do stock prediction on the scale provided\n" \
...
class InstructPage(tk.Frame):
    ...
    def main_frame(self, pNum, questions, *args):
        ...
        if pNum == 1:
            ...
            main_text = "A crosshair will appear in the center of the screen and move back and forth to the corners. \n\n" \
                        "Follow it and hold your gaze steady on the crosshair.\n\n" \
    ...
    def send_timestamp(self, qNum):
        ...
        if qNum == 2:
            ...
            print("Domain Question 1 Appearance (M" + str(2) + "): " + datetime.datetime.fromtimestamp(time.time()).strftime(
                '%Y-%m-%d %H:%M:%S.%f'))
...
class PredInterface(Frame):
    ...
    def send_final_timestamp(self):
        ...
        print("Final Page System End Marker 10: " + datetime.datetime.fromtimestamp(time.time()).strftime(
                '%Y-%m-%d %H:%M:%S.%f'))
        #fp.close()
...
class MainView(Frame):
    ...
    def send_timestamp(self, mark):
        if mark == 0:
            ...
            print("Structured Rest Page: " + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
