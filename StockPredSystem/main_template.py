...
class PageTemplate(Frame):
    ...
    def main_frame(self, pNum, q, answers, questions, *args):
        ...
        if pNum < 13:
            heading_text = "DOMAIN EVALUATION"
            qNum = pNum
            ansX = 400
        elif pNum < 24:
            heading_text = "ALGEBRA EVALUATION"
            qNum = pNum - 13
            ansX = 525

        questionFrame = tk.Frame(self, width=1400, height= 50, bg='#e6e6e6')
        questionString = tk.Label(questionFrame, bg='#e6e6e6', font=tkFont.Font(family='Helvetica', size=20, weight='bold'),
        ...
        ans1 = tk.Button(answersFrame, highlightbackground='#e6e6e6', text="A", width=20, command=lambda : self.ansSubmit("A. " + answers[0], pNum + 1, questions))
        ans2 = tk.Button(answersFrame, highlightbackground='#e6e6e6', text="B", width=20, command=lambda : self.ansSubmit("B. " + answers[1], pNum + 1, questions))
        ans3 = tk.Button(answersFrame, highlightbackground='#e6e6e6', text="C", width=20, command=lambda : self.ansSubmit("C. " + answers[2], pNum + 1, questions))
        ans4 = tk.Button(answersFrame, highlightbackground='#e6e6e6', text="D", width=20, command=lambda : self.ansSubmit("D. " + answers[3], pNum + 1, questions))
    ...
    def ansSubmit(self, answer, qNum, questions):

        # Response made
        if qNum < 14:
            self.send_timestamp_dom_resp(qNum)
        elif qNum > 13 and qNum < 25:
    ...
    def send_timestamp_dom_resp(self, qNum):
        ...
        if (qNum-1) in dom_q[0]:
            ...
            print("Domain Basic")
            print("Question " + str(qNum-1) + " Response (M" + str(2) + "): " +
                  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))

        elif (qNum-1) in dom_q[1]:
            # Send 0011 (3)
            ft232h.output(8, GPIO.LOW)
            ft232h.output(9, GPIO.LOW)
            ft232h.output(10, GPIO.HIGH)
            ...
            print("Domain Medium")
            print("Question " + str(qNum-1) + " Response (M" + str(3) + "): " +
                  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))

        elif (qNum-1) in dom_q[2]:
            # Send 0100 (4)
            ft232h.output(8, GPIO.LOW)
            ...
            print("Question " + str(qNum-1) + " Response (M" + str(4) + "): " +
                  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
    ...
    def send_timestamp_alg_resp(self, qNum):
        ...
        if (qNum - 14) in alg_q[0]:
            ...
            print("Algebra Basic")
            print("Algebra Question " + str(qNum - 14) + " Response (M" + str(6) + "): " +
                  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))

        elif (qNum - 14) in alg_q[1]:
            # Send 0111 (7)
            ft232h.output(8, GPIO.LOW)
            ft232h.output(9, GPIO.HIGH)
            ft232h.output(10, GPIO.HIGH)
            ...
            print("Algebra Question " + str(qNum - 14) + " Response (M" + str(7) + "): " +
                  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))

        elif (qNum - 14) in alg_q[2]:
            # Send 1000 (8)
            ...
            print("Algebra Question " + str(qNum - 14) + " Response (M" + str(8) + "): " +
                  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
...
class BreakPage(tk.Frame):
    ...
    def main_frame(self, pNum, questions, *args):
        if pNum == 13:
            main_text = "Thank you for completing the Domain Evaluation.\nPlease relax for 2 minutes."
        elif pNum == 35:
    ...
    def continueEval(self, qNum, questions):
        ...
        if qNum == 27 or qNum == 29 or qNum == 14:
            self.send_timestamp(qNum)

        try:
            if qNum == 25:
                print("Stock Instruct Page")
                # Stock Instruct Page
                questions[qNum - 1].show()
                self.send_timestamp(qNum)
                self.update()
                # Leading to Stock Crosshair page
                self.after(2000, self.continueEval, qNum + 1, questions)
            #elif qNum == 34:
            ...
            #    self.send_timestamp(qNum)
            #    self.update()
            #    # Leading to Stock Pred Instruct Page
            #    self.after(5000, self.continueEval, qNum + 1, questions)
                ...
                self.after(10000, self.continueEval, qNum + 1, questions)
            elif qNum == 37:
                print("Scale Page")
                # Scale Page
                questions[qNum - 1].show()
    ...
    def send_timestamp(self, qNum):
        ...
        if qNum == 14:
            ...
            print("Algebra Question " + str(1) + " Appear (M" + str(5) + "): " +
                  datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
    ...
    def send_rec_timestamp(self):
        ...
        global will_rec_timer
        will_rec_timer = Timer(10.0, self.send_rec_timestamp)
        will_rec_timer.start()
    '''
...
class ScalePage(tk.Frame):
    ...
    def send_timestamp(self, qNum):
        ...
        print("Start of Prediction Interface (M14): " + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
...
class InstructPage(tk.Frame):
    ...
    def send_timestamp(self, qNum):
        ...
        if qNum == 34:
            ...
            print("Stock Data Start (M" + str(10) + "): " + datetime.datetime.fromtimestamp(time.time()).strftime(
                '%Y-%m-%d %H:%M:%S.%f'))
            ...
            print("Crosshairs (M" + str(9) + "): " + datetime.datetime.fromtimestamp(time.time()).strftime(
                '%Y-%m-%d %H:%M:%S.%f'))
...
class CrosshairPage(tk.Frame):
    ...
    def main_frame(self, pNum, questions, *args):
        ...
        if pNum == 25 or pNum == 27 or pNum == 29 or pNum == 31 or pNum == 33:
            xnum = 0.5
            ynum = 0.5
        if pNum == 26:
            xnum = 0.01
...
class TitlePage(Page):
   def __init__(self, *args, **kwargs):
       ...
       main_label = tk.Label(self, bg='#e6e6e6', font=tkFont.Font(family='Helvetica', size=20, weight='bold'), text="The Self-Adaptive Stock Prediction Engine")
...
class MainView(Frame):
    ...
    def send_timestamp(self, mark):
        ...
        print("Question " + str(mark) + " Appear (M" + str(mark) + "): " + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
